...
class SequencerEngineTests: XCTestCase {
    ...
    ///   - frameCount: frame count for each render call. Assuming 44100 frames/sec.
    ...
    func observerTest(sequence: NoteEventSequence,
                      ...
                      renderCallCount: Int = 1) -> [TimeStampedMIDIEvent] {
        
        let eventParser = MIDI1Parser()
        ...
        var scheduledEvents = [TimeStampedMIDIEvent]()
        ...
        let block: AUScheduleMIDIEventBlock = { (sampleTime, cable, length, midiBytes) in
            ...
            let events = eventParser.parsedEvents(in: MIDIPacketData(bytes: bytes, timeStamp: timeStamp))
                .map { TimeStampedMIDIEvent(timeStamp: timeStamp, event: $0) }
    ...
    func testBasicSequence() {
        ...
        XCTAssertEqual(events[0].event, .noteOn(60, velocity: .midi1(127), channel: 0))
        XCTAssertEqual(events[0].timeStamp, 11025)
        XCTAssertEqual(events[1].event, .noteOff(60, velocity: .midi1(127), channel: 0))
    ...
    func testChord() {
        ...
        XCTAssertEqual(events.map(\.noteNumber), [60, 63, 67, 60, 63, 67])
        XCTAssertEqual(events.map(\.timeStamp), [0, 0, 0, 22050, 22050, 22050])
    }
    ...
    func testLoop() {
        ...
        XCTAssertEqual(events.map(\.noteNumber),
    ...
    func testOverlap() {
        ...
        XCTAssertEqual(events.map(\.noteNumber), [60, 63, 63, 60])
        XCTAssertEqual(events.map(\.timeStamp), [0, 2205, 4410, 22050])
    }
    ...
    func testSameNoteRepeating() {
        ...
        XCTAssertEqual(events.map(\.noteNumber), [60, 60, 60, 60])
        XCTAssertEqual(events.map(\.status), [.noteOn, .noteOff, .noteOn, .noteOff])
        XCTAssertEqual(events.map(\.timeStamp), [0, 22050, 22050, 33075])
    }
    ...
    func testSameNoteRepeatingInChords() {
        ...
        XCTAssertEqual(events.map(\.noteNumber), [60, 62, 64, 60, 62, 64, 61, 64, 62, 61, 64, 62])
        XCTAssertEqual(events.map(\.status), [.noteOn, .noteOn, .noteOn, .noteOff, .noteOff, .noteOff,
                                              .noteOn, .noteOn, .noteOn, .noteOff, .noteOff, .noteOff])
        XCTAssertEqual(events.map(\.timeStamp), [0, 0, 0, 22050, 22050, 22050, 22050, 22050, 22050, 33075, 33075, 33075])
    }
    ...
    func testSameNoteRepeatingInChordsAcrossLoop() {
        ...
        XCTAssertEqual(events.map(\.noteNumber), [60, 62, 64, 60, 62, 64,
                                                  60, 62, 64, 60, 62, 64,
                                                  60, 62, 64, 60, 62, 64,
                                                  60, 62, 64, 60, 62, 64])
        XCTAssertEqual(events.compactMap(\.status), [.noteOn, .noteOn, .noteOn, .noteOff, .noteOff,.noteOff,
                                                     .noteOn, .noteOn, .noteOn, .noteOff, .noteOff,.noteOff,
                                                     .noteOn, .noteOn, .noteOn, .noteOff, .noteOff,.noteOff,
                                                     .noteOn, .noteOn, .noteOn, .noteOff, .noteOff,.noteOff])
    ...
    func testShortNotesAcrossLoop() {
        ...
        XCTAssertEqual(events.map(\.noteNumber), [60, 62, 65, 60, 62, 65,
...
struct TimeStampedMIDIEvent {
    var timeStamp: CoreMIDITimeStamp
    var event: MIDIEvent
    
    var noteNumber: UInt8? {
        switch event {
        case let .noteOn(payload):
            return payload.note.number.uInt8Value
    ...
    var status: MIDI1Status? {
        MIDI1Status(of: event)
    }
}
...
enum MIDI1Status {
    case noteOff
    case noteOn
    case notePressure // poly aftertouch
    case cc
    case programChange
    case pitchBend
    case pressure // channel pressure
    
    init?(of event: MIDIEvent) {
        guard let statusByte = event.midi1RawStatusByte() else { return nil }
        self.init(statusNibble: statusByte.nibbles.high)
    }
    
    init?(statusNibble: UInt4) {
        switch statusNibble {
        case 0x0...0x7: return nil // not a status byte
        case 0x8: self = .noteOff
        case 0x9: self = .noteOn
        case 0xA: self = .notePressure
        case 0xB: self = .cc
        case 0xC: self = .programChange
        case 0xD: self = .pressure
        case 0xE: self = .pitchBend
        case 0xF: return nil // system message
        default: return nil
        }
    }
}
